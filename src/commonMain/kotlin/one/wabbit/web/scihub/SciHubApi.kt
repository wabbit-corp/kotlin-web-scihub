...
sealed class SciHubError(message: String, cause: Throwable? = null) : Exception(message, cause) {
    ...
    class MirrorDiscovery(val provider: String, cause: Throwable) :
        SciHubError("Mirror discovery failed for $provider: ${cause::class.simpleName}: ${cause.message}", cause)
    ...
    class Network(val url: String, cause: Throwable) :
        SciHubError("Network failure talking to $url: ${cause::class.simpleName}: ${cause.message}", cause)
    ...
    class Captcha(val mirror: String, val query: SciHubQuery, val bodySample: String) :
        SciHubError("Captcha or anti-bot challenge on $mirror for ${query.value}: ${bodySample.take(256)}")

    class Parse(val mirror: String, val query: SciHubQuery, val bodySample: String) :
        SciHubError("Could not extract PDF link from $mirror for ${query.value}: ${bodySample.take(256)}")
        ...
        SciHubError(
            buildString {
                ...
                if (errors.isNotEmpty()) {
                    append(": ")
                    append(errors.joinToString("; ") { it.message ?: it::class.simpleName ?: "SciHubError" })
                }
            },
        )
}
...
interface SciHubApi {
    data class Config(
        ...
        val onMirrorRefresh: ((duration: Duration, success: Boolean, error: SciHubError?) -> Unit)? = null,
    ) {
        init {
    ...
    interface Cache {
        ...
        companion object {
            fun inMemory(clock: Clock = Clock.System): Cache = object : Cache {
                private val mutex = Mutex()
                private var snapshot: MirrorCacheEntry? = null

                override suspend fun getMirrors(): MirrorCacheEntry? = mutex.withLock { snapshot }

                override suspend fun putMirrors(entry: MirrorCacheEntry) {
                    mutex.withLock {
                        val previous = snapshot?.mirrors.orEmpty().associateBy { it.baseUrl }
                        val merged = entry.mirrors.map { current ->
...
class YoviSunMirrorProvider(
    private val sourceUrl: String = "http://tool.yovisun.com/scihub/",
...
class StaticMirrorProvider(
    ...
    override suspend fun discover(
        httpClient: HttpClient,
        config: SciHubApi.Config,
    ): List<Mirror> = urls.mapNotNull { raw -> normalizeMirrorBaseUrl(raw) }.distinct().map { Mirror(it, name) }
}

open class RegexMirrorProvider(
    ...
    override suspend fun discover(
        ...
        if (!response.status.isSuccess()) {
            throw SciHubError.MirrorDiscovery(
                name,
                SciHubError.Http(sourceUrl, response.status.value, response.safeBodyPrefix(2048)),
            )
        }
        ...
        if (mirrors.isEmpty()) {
            throw SciHubError.MirrorDiscovery(name, SciHubError.Parse(sourceUrl, SciHubQuery.Title(name), collapseWhitespace(html)))
        }
        return mirrors
    }
}
...
class KtorSciHubApi(
    ...
    private suspend fun discoverMirrors(): List<Mirror> {
        ...
        return if (policy == null) discoverMirrorsOnce() else runWithRetry(policy) { discoverMirrorsOnce() }
    }
    ...
    private suspend fun discoverMirrorsOnce(): List<Mirror> {
        ...
        for (provider in config.mirrorProviders) {
            try {
                ...
                if (t is CancellationException) throw t
                val error = if (t is SciHubError) t else SciHubError.MirrorDiscovery(provider.name, t)
    ...
    private suspend fun resolveAgainstMirror(
        ...
        val response = try {
            httpClient.post(lookupUrl) {
                ...
                setBody(
                    FormDataContent(
                        Parameters.build {
                            append("request", query.requestValue())
                        },
                    ),
                )
            }
        ...
        return when (val parsed = SciHubParser.parse(query, mirrorBaseUrl, body)) {
            ...
            is SciHubParser.ParseResult.NotFound -> throw SciHubError.NotFound(mirrorBaseUrl, query, parsed.sample)
            is SciHubParser.ParseResult.Captcha -> throw SciHubError.Captcha(mirrorBaseUrl, query, parsed.sample)
            is SciHubParser.ParseResult.Suspicious -> throw SciHubError.Parse(mirrorBaseUrl, query, parsed.sample)
        }
    }
    ...
    fun parse(
        ...
        if (pdfRaw == null) {
            ...
            return when {
                ...
                "captcha" in lower || "капчу" in lower || "altcha" in lower -> ParseResult.Captcha(sample)
                else -> ParseResult.Suspicious(sample)
            }
        }

        val title = extractMetaContents(document, "citation_title").firstOrNull() ?: extractTitle(document)
...
private fun mergeMirrors(
    discovered: List<Mirror>,
    previous: List<MirrorEntry>,
): List<MirrorEntry> {
...
private fun normalizeMirrorBaseUrl(raw: String): String? {
    ...
    val match = Regex("""^(https?://)(?:www\.)?([A-Za-z0-9.-]+)$""").matchEntire(value) ?: return null
...
private fun normalizePdfUrl(mirrorBaseUrl: String, raw: String): String {
    val trimmed = decodeHtmlEntities(raw.trim())
    return when {
        trimmed.startsWith("https://") || trimmed.startsWith("http://") -> trimmed.substringBefore('#')
...
private fun collapseWhitespace(text: String): String = text.replace(Regex("""\s+"""), " ").trim().take(512)
